...
import { AccordionComponent, ButtonComponent, CheckboxComponent, SwitchComponent, TextComponent } from 'ui';

@Component({
  selector: 'app-accordion-settings-form-example',
  standalone: true,
  imports: [FormsModule, AccordionComponent, ButtonComponent, CheckboxComponent, SwitchComponent, TextComponent],
  template: `
    <div
    ...
    >
      <div style="display:flex;flex-direction:column;gap:0.25rem">
        ...
        <div style="font-size:0.8125rem;color:var(--color-neutral-foreground2-rest)">
          A practical accordion often groups related form controls into compact sections instead of acting as a purely
      ...
      <ui-accordion label="Delivery channels" icon="mail">
        <div style="display:flex;flex-direction:column;gap:0.75rem">
          <ui-switch label="Email updates" [(ngModel)]="emailUpdates" [ngModelOptions]="{ standalone: true }" />
          <ui-switch label="Push alerts" [(ngModel)]="pushAlerts" [ngModelOptions]="{ standalone: true }" />
          <ui-checkbox label="Digest only" [(ngModel)]="digestOnly" [ngModelOptions]="{ standalone: true }" />
        </div>
      </ui-accordion>
