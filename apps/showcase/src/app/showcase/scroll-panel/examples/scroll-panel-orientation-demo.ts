        ...
        >
          <div style="display:flex;gap:0.75rem;width:max-content;padding-bottom:0.25rem">
            @for (item of horizontalItems; track item.id) {
              <div
                style="width:11rem;padding:0.875rem 1rem;border:1px solid var(--color-neutral-stroke-rest);border-radius:0.875rem;background:var(--color-neutral-background1-rest)"
              >
        ...
        >
          ...
          >
            @for (item of canvasItems; track item.id) {
              <div
                style="height:7rem;padding:0.875rem 1rem;border:1px solid var(--color-neutral-stroke-rest);border-radius:0.875rem;background:linear-gradient(180deg,var(--color-neutral-background1-rest),var(--color-neutral-background2-rest))"
              >
                <div style="font-size:0.875rem;font-weight:600">{{ item.title }}</div>
                <div
                  style="margin-top:0.25rem;font-size:0.8125rem;color:var(--color-neutral-foreground2-rest)"
