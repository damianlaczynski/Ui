...
import { DataGridInteractiveDemoComponent } from './examples/data-grid-interactive-demo';
...
const componentMap = {
  ...
  interactiveDemo: DataGridInteractiveDemoComponent,
} as const;

export const DATA_GRID_DOC_ASSET_PATHS: ShowcaseDocAssetPaths = {
  markdown: `/llms/${dataGridMeta.slug}.md`,
  ...(Object.fromEntries(
    Object.entries(dataGridMeta.snippets).map(([key, file]) => [
      key,
      `/llms/snippets/${dataGridMeta.slug}/${file}`,
    ]),
