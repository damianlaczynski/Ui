    ...
    <div style="display:flex;flex-direction:column;gap:1.25rem">
      <div style="display:flex;flex-wrap:wrap;gap:1.25rem">
        <div style="display:flex;flex-direction:column;gap:0.5rem;min-width:10rem">
          ...
          <ui-rating [value]="shortScale()" [max]="5" [showValue]="true" (valueChange)="shortScale.set($event)" />
        </div>
        <div style="display:flex;flex-direction:column;gap:0.5rem;min-width:10rem">
          <span style="font-size:0.75rem;color:var(--color-neutral-foreground2-rest)">Ten stars</span>
          <ui-rating [value]="longScale()" [max]="10" [showValue]="true" (valueChange)="longScale.set($event)" />
        </div>
      </div>
